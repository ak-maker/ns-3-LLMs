...
def remove_headerlink(lines):
    pattern = re.compile(r'<a[^>]*class="headerlink"[^>]*>.*?</a>')
    for line in lines:
        cleaned_line = pattern.sub('', line)
        if cleaned_line == '':
            continue
...
def remove_toc(lines):
    is_start = False
    start_pattern = re.compile(r'## Table of Contents')
...
def multiline_match(stream, patterns):
    ...
    buffer = []
    for line in stream:
        if len(buffer) < len(patterns) and patterns[len(buffer)].match(line):
            buffer.append(line)
            if len(buffer) == len(patterns):
                buffer = []
            continue
...
def process_one_file(file_path: Path):
    with MarkdownReader(file_path) as reader:
        lines = list(reader)
...
@app.command()
def cli(
    input_path: Annotated[Path, typer.Argument(help="An input markdown file or a directory containing markdown files")],
):
    """
    Clean markdown files in-place.
    """
    assert input_path.exists(), f"Input file/directory {input_path} doesn't exist."
    if input_path.is_file():
        process_one_file(input_path)
    else:
        import glob
        files = glob.glob(f"{input_path}/**/*.md", recursive=True)
        for file_path in track(
                files,
                description=f"Cleaning {len(files)} markdown files from {input_path}"):
            process_one_file(Path(file_path))
    log.info(f"{Path(__file__).stem} completed. Markdown files have been updated in-place.")
